...
function LoginScreen({ onLogin }) {
  ...
  async function doLogin() {
    ...
    if (!u.password) return setMsg({ text: 'No password set yet — ask your manager to set one for you', type: 'error' });
    ...
    <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: T.surface, borderRadius: 24, padding: 40, maxWidth: 400, width: '100%', border: `1px solid ${T.border}` }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input value={un} onChange={e => setUn(e.target.value)} placeholder="your.username" disabled={busy}
        ...
        <div style={{ marginBottom: 24, position: 'relative' }}>
          ...
          <input type={show ? 'text' : 'password'} value={pw} onChange={e => setPw(e.target.value)}
            onKeyPress={e => e.key === 'Enter' && doLogin()} placeholder="••••••••" disabled={busy}
        ...
        <Btn onClick={doLogin} disabled={!un.trim() || !pw.trim() || busy} style={{ width: '100%', justifyContent: 'center' }}>
...
export default function App() {
  ...
  useEffect(() => {
    async function restore() {
      try {
        const s = await sGet('lastSession:sw');
  ...
  async function handleLogin(u) {
    await sSet('lastSession:sw', { username: u.username });
    setUser(u);
    // if they haven't picked a pet yet, send them to pet picker
    if (!u.petName) setShowPetPicker(true);
  }

  async function handlePetPickerComplete(petData) {
    const updated = { ...user, ...petData };
    await sSet('user:' + updated.username, updated);
    // update in allSWs list too
    const all = await sGet('allSWs') || [];
    await sSet('allSWs', all.map(s => s.username === updated.username ? updated : s));
    setUser(updated);
