...
function LoginScreen({ onLogin, onFirstLogin }) {
  ...
  // null = username step, 'new' = create password, 'existing' = enter password
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: T.surface, borderRadius: 24, padding: 40, maxWidth: 400, width: '100%', border: `1px solid ${T.border}` }}>
        ...
        {step === 'username' && (
          <>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input value={un} onChange={e => setUn(e.target.value)} onKeyPress={e => e.key === 'Enter' && lookupUser()} placeholder="your.username"
            ...
            <Btn onClick={lookupUser} disabled={!un.trim()} style={{ width: '100%', justifyContent: 'center' }}>Next →</Btn>
          </>
        )}
        ...
        {step === 'existing' && (
          <>
            <button onClick={() => { setStep('username'); setPw(''); setMsg(null); }} style={{ background: 'none', border: 'none', color: T.teal, cursor: 'pointer', fontFamily: 'DM Sans', fontSize: 13, padding: 0, marginBottom: 16 }}>← Back</button>
            <h1 style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: 24, color: T.text, marginBottom: 4 }}>Welcome back!</h1>
            <p style={{ color: T.muted, marginBottom: 24, fontSize: 13 }}>@{foundUser?.username}</p>
            <Msg text={msg?.text} type={msg?.type} />
            <div style={{ marginBottom: 24, position: 'relative' }}>
              <label style={{ color: T.textDim, fontSize: 12, display: 'block', marginBottom: 6 }}>Password</label>
              <input type={show ? 'text' : 'password'} value={pw} onChange={e => setPw(e.target.value)} onKeyPress={e => e.key === 'Enter' && doLogin()} placeholder="••••••••"
        ...
        {step === 'new' && (
          <>
            <button onClick={() => { setStep('username'); setPw(''); setConfirm(''); setMsg(null); }} style={{ background: 'none', border: 'none', color: T.teal, cursor: 'pointer', fontFamily: 'DM Sans', fontSize: 13, padding: 0, marginBottom: 16 }}>← Back</button>
            <div style={{ background: `${T.teal}15`, border: `1px solid ${T.teal}40`, borderRadius: 12, padding: '12px 14px', marginBottom: 20 }}>
              ...
              <div style={{ color: T.textDim, fontSize: 12 }}>Welcome @{foundUser?.username}! Set a password to secure your account.</div>
            </div>
            <Msg text={msg?.text} type={msg?.type} />
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} onKeyPress={e => e.key === 'Enter' && doCreatePassword()} placeholder="••••••••"
            ...
            <Btn onClick={doCreatePassword} disabled={!pw.trim() || !confirm.trim()} style={{ width: '100%', justifyContent: 'center' }}>
