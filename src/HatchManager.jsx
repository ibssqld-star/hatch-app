...
function CreateUserModal({ type, onClose, onCreated }) {
  ...
  async function create() {
    ...
    const u = { username:un.trim(), email:em.trim(), role:type, password:null, createdAt:Date.now(), petName:null, petType:null, petColor:null, tickets:0, petInventory:[], assignedSW:null };
    ...
    setBusy(false);
    setMsg({ text:`✅ ${un} created! They can log in and set up their pet.`, type:'success' });
    onCreated?.();
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <p style={{ color:T.muted, fontSize:12, marginBottom:16 }}>They'll set their password and pick a pet on first login</p>
      <Msg text={msg?.text} type={msg?.type} />
